...
class CSVPlotter:
    ...
    # Creating a widget in a separate canvas and adding it to the application
    ...
    def update_plot( self, event=None ):
        ...
        self.ax.clear()
        if plot_type == 'Line Plot': self.ax.plot( self.df[x], self.df[y], label=f'{y} vs {x}' )
        elif plot_type == 'Bar Plot': self.ax.bar( self.df[x], self.df[y], self.df[y], label=f'{y} vs {x}' )
        elif plot_type == 'Scatter Plot': self.ax.scatter( self.df[x], self.df[y], self.df[y], label=f'{y} vs {x}' )
